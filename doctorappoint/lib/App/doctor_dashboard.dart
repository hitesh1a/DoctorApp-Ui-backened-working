...
class _DoctorDashboardState extends State<DoctorDashboard> {
                          ...
                          return AlertDialog(
                            ...
                            actions: <Widget>[
                              TextButton(
                                child: Text('Confirm'),
                                onPressed: () {
                                  changestatus(appointment['_id'], index, appointment['pname']);
  ...
  Future<void> removeAppointmentFromUser(String userId, String appointmentId) async {
    try {
      final apiUrl = Uri.parse('$api/api/user/removeAppointment/$userId/$appointmentId');
      final response = await http.delete(apiUrl);

      if (response.statusCode == 200) {
        print('Appointment removed from user section');
      } else {
        throw Exception('Failed to remove appointment from user section. Status code: ${response.statusCode}');
      }
    } catch (e) {
      print('Error removing appointment from user section: $e');
    }
  }

  void changestatus(String id, int index, String patientName) async {
    showDialog(
      context: context,
      builder: (c) {
        return LoadingDialog(
    ...
    try {
      ...
      final response = await http.put(
        apiroute,
        headers: headers,
        body: jsonEncode({"status": "done"}), // Ensure correct payload
      );
      ...
      if (response.statusCode == 200 && jsonResponse['msg'] == 'Success') {
        ...
        // Remove appointment from the user section
        if (userId != null) {
          await removeAppointmentFromUser(userId, id);
        }
        ...
        AwesomeDialog(
          ...
          title: 'Success',
          desc: 'Appointment status updated successfully',
          btnOkOnPress: () {},
        ).show();
      } else {
        ...
        AwesomeDialog(
          ...
          animType: AnimType.rightSlide,
          title: 'Warning',
          desc: 'Update failed: ${jsonResponse['msg']}',
          btnOkOnPress: () {},
      ...
      AwesomeDialog(
        ...
        animType: AnimType.rightSlide,
        title: 'Error',
        desc: 'An error occurred: $e. Please try again!',
        btnCancelOnPress: () {},
      ).show();
    }
  }
